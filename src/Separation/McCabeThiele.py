...
class McCabeThiele:
    ...
    def make_equilibrium_points(self):
        ...
        while eqs[n-1] < self.xd and n < 2*self.max_eq_array_size-5:
            eqs[n] = self.make_equilibrium_curve(eqs[n-1])
            strip = (eqs[n] - self.stripping_line[1]) / self.stripping_line[0]
            rect = (eqs[n] - self.rectifying_line[1]) / self.rectifying_line[0]
